<?php
          $show = $_POST['show'];
          $namatable = "insertpesertakelas";
          $act="pesertaimport";
          $berhasil=$belum=$sudahada=$gagal=0;
          $query = "select * from progress where act='InsertPesertaKelasKuliah'";
          ...
          $headertable = "<table class='table table-striped'  border='1' style='font-size:80%'><tr>
          <th>No</th><th>NIM</th><th>Nama</th><th>Prodi</th><th>Kode MK</th><th>Kelas</th>
          <th>Semester</th><th>ID Kelas Kuliah</th><th>Error</th><th>Description</th></tr>";
          ...
          if(mysqli_num_rows($hasil) > 0 ){
                    ...
                    echo "<tr><td><pre>" . $no++;
                    echo "</td><td><pre>" . $x['nim'];echo "</td><td>" . $x['nama_mahasiswa'];
                    echo "</td><td style='font-size:80%'><pre>" . $x['nama_prodi'];echo "</td><td><pre>" . $x['kode_mata_kuliah'];
                    echo "</td><td><pre>" . $x['kode_kelas'];
                    echo "</td><td><pre>" . $x['semester'];echo "</td><td><pre>" . $x['id_kelas_kuliah'];
          ...
          $total=$berhasil+$belum+$gagal; 
                    ...
                    echo "</td><td><pre>" . $x['nama_prodi'];echo "</td><td><pre>" . $x['kode_mata_kuliah'];
                    echo "</td><td><pre>" . $x['kode_kelas'];
                    echo "</td><td><pre>" . $x['semester'];echo "</td><td><pre>" . $x['id_kelas_kuliah'];
                    echo "</td><td><pre>" . $x['err_no'];echo "</td><td><code>" . $x['err_desc'] . "</code></td></tr>";
                    }
            ...
            $tampil = '<a href="?module=inject&act='.$act.'&show=no"><button type="button" class="btn btn-danger">
            SEMBUNYIKAN DATA <span class="badge bg-transparent"></span></button></a>';
            $query = "select * from ".$namatable." where err_no is null or err_no = ''";
            $hasil = mysqli_query($db, $query);
            if(mysqli_num_rows($hasil) > 0 ){
            echo $headertable;
            while($x = mysqli_fetch_array($hasil)){
